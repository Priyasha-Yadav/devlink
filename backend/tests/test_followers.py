...
def test_follow_already_following(client: TestClient, register_and_login):
    ...
    response = client.post(f"/api/followers/{uid2}", headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 400
...
def test_unfollow_user(client: TestClient, register_and_login):
    ...
    client.post(f"/api/followers/{uid2}", headers={"Authorization": f"Bearer {token1}"})
    response = client.delete(f"/api/followers/{uid2}", headers={"Authorization": f"Bearer {token1}"})
...
def test_my_following(client: TestClient, register_and_login):
    ...
    response = client.get("/api/followers/", headers={"Authorization": f"Bearer {token1}"})
...
def test_is_following(client: TestClient, register_and_login):
    ...
    response = client.get(f"/api/followers/{uid2}/is-following", headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 200
...
def test_mutual_followers(client: TestClient, register_and_login):
    ...
    response = client.get(f"/api/followers/mutual/{uid2}", headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 200
