...
def test_get_notification(client: TestClient, register_and_login):
    ...
    c = client.post("/api/notifications/", json={"recipient_id": uid2, "type": "message", "title": "T", "message": "M" + str(uuid.uuid4())}, headers={"Authorization": f"Bearer {token1}"})
    nid = c.json()["id"]
    
    response = client.get(f"/api/notifications/{nid}", headers={"Authorization": f"Bearer {token1}"})
...
def test_get_notification_not_found(client: TestClient, register_and_login):
    ...
    response = client.get(f"/api/notifications/{uuid.uuid4()}", headers={"Authorization": f"Bearer {token}"})
...
def test_list_notifications(client: TestClient, register_and_login):
    ...
    client.post("/api/notifications/", json={"recipient_id": uid2, "type": "message", "title": "T1", "message": "M" + str(uuid.uuid4())}, headers={"Authorization": f"Bearer {token1}"})
    client.post("/api/notifications/", json={"recipient_id": uid2, "type": "follow", "title": "T2", "message": "M" + str(uuid.uuid4())}, headers={"Authorization": f"Bearer {token1}"})
    
    response = client.get("/api/notifications/", headers={"Authorization": f"Bearer {token2}"})
    assert response.status_code == 200
...
def test_unread_notifications(client: TestClient, register_and_login):
    ...
    response = client.get("/api/notifications/unread", headers={"Authorization": f"Bearer {token2}"})
    assert response.status_code == 200
...
def test_unread_count(client: TestClient, register_and_login):
    ...
    client.post("/api/notifications/", json={"recipient_id": uid2, "type": "message", "title": "T1", "message": "M1"}, headers={"Authorization": f"Bearer {token1}"})
    client.post("/api/notifications/", json={"recipient_id": uid2, "type": "follow", "title": "T2", "message": "M2"}, headers={"Authorization": f"Bearer {token1}"})
    
    response = client.get("/api/notifications/unread/count", headers={"Authorization": f"Bearer {token2}"})
    assert response.status_code == 200
...
def test_mark_as_read(client: TestClient, register_and_login):
    ...
    c = client.post("/api/notifications/", json={"recipient_id": uid2, "type": "message", "title": "T1", "message": "M" + str(uuid.uuid4())}, headers={"Authorization": f"Bearer {token1}"})
    nid = c.json()["id"]
    
    response = client.patch(f"/api/notifications/{nid}/read", headers={"Authorization": f"Bearer {token2}"})
    assert response.status_code == 200
...
def test_mark_as_read_not_found(client: TestClient, register_and_login):
    ...
    response = client.patch(f"/api/notifications/{uuid.uuid4()}/read", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 404
...
def test_mark_all_as_read(client: TestClient, register_and_login):
    ...
    response = client.patch("/api/notifications/read-all", headers={"Authorization": f"Bearer {token2}"})
    assert response.status_code == 200
    
    unread = client.get("/api/notifications/unread/count", headers={"Authorization": f"Bearer {token2}"})
...
def test_update_notification(client: TestClient, register_and_login):
    ...
    response = client.put(f"/api/notifications/{nid}", json={"is_read": True}, headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 200
    assert response.json()["is_read"] is True
...
def test_update_notification_not_found(client: TestClient, register_and_login):
    ...
    response = client.put(f"/api/notifications/{uuid.uuid4()}", json={"is_read": True}, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 404
...
def test_delete_notification(client: TestClient, register_and_login):
    ...
    response = client.delete(f"/api/notifications/{nid}", headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 204
    
    get_resp = client.get(f"/api/notifications/{nid}", headers={"Authorization": f"Bearer {token1}"})
...
def test_delete_notification_not_found(client: TestClient, register_and_login):
    ...
    response = client.delete(f"/api/notifications/{uuid.uuid4()}", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 404
